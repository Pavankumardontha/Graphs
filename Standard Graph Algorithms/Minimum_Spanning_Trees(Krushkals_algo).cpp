...
/*
Given a weighted undirected graph. Find the sum of weights of edges of a Minimum Spanning Tree.A minimum spanning tree (MST) 
or minimum weight spanning tree is a subset of the edges of a connected, edge-weighted undirected graph that connects all 
the vertices together, without any cycles and with the minimum possible total edge weight.

Input:
Given 2 integers N and M. N represents the number of vertices in the graph. M represents the number of edges between any 2 
vertices.Then M lines follow, each line has 3 space separated integers  , ,  where  and  represents an edge from a vertex  
to a vertex  and  respresents the weight of that edge.
...
int main()
{
	...
	/* initially all the nodes point to themselves and they are parents of themselves.*/
    	...
    	for(int i=0;i<m;i++)
    	{
     		/*we have m edges with us now.we have also sorted the edges depending on their weights.So now lets proceed.
       		we will find the parents of the first edge*/
     		/*we have 2 choices for each edge,whether it can be included or cannot be included.
       		So we have only 2 sets here,the included set and the not included set.We can either 
	 	include an edge or not include.*/
     		int k=find(g[i].a);
     		int l=find(g[i].b);
	 	...
	 	if(k!=l)//if both the node parents are different then we should add the edge into the Minimum spanning tree edge-list
	 	{
			...
			/* 
   			here its very important to note that we are merging the parents of the nodes and NOT the nodes. Mergeing the parents 
      			is different than megering the nodes. Whenever we merge the parents, we are mergeing 2 large groups which have 
	 		different parents by making their parents same. If parents of any 2 nodes are same then it means that we can reach one 
    			node via another through some path. If they are not reachable then their parents will be different. Reachable nodes have 
       			the same parents. !! 
			*/
